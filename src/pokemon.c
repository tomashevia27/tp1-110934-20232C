...
bool lineas_nulas(char linea_pokemon[MAX_CARACTERES],
		  char lineas_ataques[MAX_ATAQUES][MAX_CARACTERES],
		  FILE *archivo)
{
	return strlen(linea_pokemon) == 0 || strlen(lineas_ataques[0]) == 0 ||
	       strlen(lineas_ataques[1]) == 0 || strlen(lineas_ataques[2]) == 0;
}

void asignar_y_contar_datos(char linea_pokemon[MAX_CARACTERES],
			    char lineas_ataques[MAX_ATAQUES][MAX_CARACTERES],
			    int *datos_leidos, pokemon_t *nuevo_pokemon,
			    char *tipo_pokemon, char tipos_ataques[MAX_ATAQUES])
{
...
informacion_pokemon_t *pokemon_cargar_archivo(const char *path)
{
	...
	while (seguir_leyendo) {
		...
		char lineas_ataques[MAX_ATAQUES][MAX_CARACTERES];
		if (fscanf(archivo, "%s\n%s\n%s\n%s\n", linea_pokemon,
			   lineas_ataques[0], lineas_ataques[1],
			   lineas_ataques[2]) == 4) {
			if (!lineas_nulas(linea_pokemon, lineas_ataques,
					  archivo)) {
				pokemon_t *nuevo_pokemon =
					malloc(sizeof(pokemon_t));
				char tipo_pokemon;
				char tipos_ataques[MAX_ATAQUES];
				int datos_leidos = 0;
				asignar_y_contar_datos(
					linea_pokemon, lineas_ataques,
					&datos_leidos, nuevo_pokemon,
					&tipo_pokemon, tipos_ataques);
				if (datos_leidos == 11 &&
				    validar_tipo(tipo_pokemon) &&
				    validar_tipo(tipos_ataques[0]) &&
				    validar_tipo(tipos_ataques[1]) &&
				    validar_tipo(tipos_ataques[2])) {
					(info_pokemon->cantidad_pokemones)++;
					pokemon_t **puntero_auxiliar = realloc(
						info_pokemon->pokemones,
						(long unsigned int)(info_pokemon
									    ->cantidad_pokemones) *
							sizeof(pokemon_t));
